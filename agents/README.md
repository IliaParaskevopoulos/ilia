...
— read that first, then run each module against the same three inputs
...
— a reader should be able to tell at a glance which numbers are solid and
which are placeholders, without opening every module.
...
five modules of tables and assumption logs is easy to skim visually and
hard to skim as scrolling text.
